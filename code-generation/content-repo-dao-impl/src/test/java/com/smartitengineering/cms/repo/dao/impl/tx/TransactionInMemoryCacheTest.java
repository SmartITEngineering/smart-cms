...
public class TransactionInMemoryCacheTest {
  ...
  private Injector isolatedOnlyInjector;
  ...
  public void setup() {
    ...
    isolatedOnlyInjector = Guice.createInjector(new AbstractModule() {
      ...
      protected void configure() {
        bind(boolean.class).annotatedWith(Names.named("nonIsolatedLookupEnabled")).toInstance(Boolean.FALSE);
  ...
  @Test
  public void testInitializationInIsolatedOnlyMode() {
    TransactionInMemoryCache memCache = isolatedOnlyInjector.getInstance(TransactionInMemoryCache.class);
    Assert.assertNotNull(memCache);
    TransactionFactory factory = isolatedOnlyInjector.getInstance(TransactionFactory.class);
    Assert.assertNotNull(factory);
  }
  ...
  @Test
  public void testNonIsolatedSettingDisabledIsolationLookup() throws Exception {
    final TransactionInMemoryCache memCache = isolatedOnlyInjector.getInstance(TransactionInMemoryCache.class);
    final TransactionFactory factory = isolatedOnlyInjector.getInstance(TransactionFactory.class);
    DemoDomain d0 = new DemoDomain();
    DemoDomain d1 = new DemoDomain();
    DemoDomain d2 = new DemoDomain();
    d0.setTestValue(0);
  ...
  public void testNonIsolatedSettingDisabledNonIsolationLookup() throws Exception {
    final TransactionInMemoryCache memCache = isolatedOnlyInjector.getInstance(TransactionInMemoryCache.class);
    final TransactionFactory factory = isolatedOnlyInjector.getInstance(TransactionFactory.class);
    DemoDomain d0 = new DemoDomain();
    DemoDomain d1 = new DemoDomain();
